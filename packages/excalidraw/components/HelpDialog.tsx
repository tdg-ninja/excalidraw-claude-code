...
const HelpSearchContext = React.createContext("");

const matchesSearchTerm = (label: string, searchTerm: string) =>
  !searchTerm || label.toLowerCase().includes(searchTerm);
...
const ShortcutIsland = (props: {
  ...
  const searchTerm = React.useContext(HelpSearchContext);

  const hasVisibleShortcuts = React.Children.toArray(props.children).some(
    (child) =>
...
export const HelpDialog = ({ onClose }: { onClose?: () => void }) => {
  ...
  const handleKeyDown = React.useCallback((event: React.KeyboardEvent) => {
    if (event[KEYS.CTRL_OR_CMD] && event.key.toLowerCase() === KEYS.F) {
    ...
    <>
      ...
      >
        <div onKeyDown={handleKeyDown}>
          ...
          />
          <HelpSearchContext.Provider value={searchTerm}>
